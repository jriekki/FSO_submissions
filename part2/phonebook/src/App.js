...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if (persons.every(inequalName)){
      personService.addPerson(personObject).then(response => setPersons(persons.concat(response)))
      ...
      if (window.confirm(`${personObject.name} is already in the phonebook. Update the number?`))
        ...
        personService.updatePerson(idMatch,personObject).then(response => {
          setPersons(persons.map(p => inequalName(p) ? p : response))
          ...
          console.log(error)
          setPersons(persons.filter(inequalName))
          setNotificationStyle(negativeStyle)
          setAddedMessage(`Information of ${personObject.name} has already been removed from the server`)  
        })
